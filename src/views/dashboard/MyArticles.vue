            ...
            <div class="articles__article" v-for="article in filteredArticles" :key="article.id">
                ...
                <div class="article__content">
                    ...
                    <div class="article__info">
                        ...
                        <div v-if="article.reviewer != ''" class="article-info-item article-info__author">
                            <svg xmlns="http://www.w3.org/2000/svg" height="48" viewBox="0 96 960 960" width="48"><path d="M480 575q-66 0-108-42t-42-108q0-66 42-108t108-42q66 0 108 42t42 108q0 66-42 108t-108 42ZM160 896v-94q0-38 19-65t49-41q67-30 128.5-45T480 636q62 0 123 15.5t127.921 44.694q31.301 14.126 50.19 40.966Q800 764 800 802v94H160Zm60-60h520v-34q0-16-9.5-30.5T707 750q-64-31-117-42.5T480 696q-57 0-111 11.5T252 750q-14 7-23 21.5t-9 30.5v34Zm260-321q39 0 64.5-25.5T570 425q0-39-25.5-64.5T480 335q-39 0-64.5 25.5T390 425q0 39 25.5 64.5T480 515Zm0-90Zm0 411Z"/></svg>
                ...
                <hr v-if="!isMobile" class="article-separator">
                <div v-if="user.role != 'Student'" class="article-favorite" @click="article.isSaved = !article.isSaved">
                ...
                <div v-if="user.role == 'Student'" class="article-status">
                    <svg v-if="article.publishStatus == 'Accepted'" class="article-status-accepted" xmlns="http://www.w3.org/2000/svg" height="48" viewBox="0 96 960 960" width="48"><path d="m421 758 283-283-46-45-237 237-120-120-45 45 165 166Zm59 218q-82 0-155-31.5t-127.5-86Q143 804 111.5 731T80 576q0-83 31.5-156t86-127Q252 239 325 207.5T480 176q83 0 156 31.5T763 293q54 54 85.5 127T880 576q0 82-31.5 155T763 858.5q-54 54.5-127 86T480 976Zm0-60q142 0 241-99.5T820 576q0-142-99-241t-241-99q-141 0-240.5 99T140 576q0 141 99.5 240.5T480 916Zm0-340Z"/></svg>
                    <svg v-if="article.publishStatus == 'Declined'" class="article-status-declined" xmlns="http://www.w3.org/2000/svg" height="48" viewBox="0 96 960 960" width="48"><path d="m330 768 150-150 150 150 42-42-150-150 150-150-42-42-150 150-150-150-42 42 150 150-150 150 42 42Zm150 208q-82 0-155-31.5t-127.5-86Q143 804 111.5 731T80 576q0-83 31.5-156t86-127Q252 239 325 207.5T480 176q83 0 156 31.5T763 293q54 54 85.5 127T880 576q0 82-31.5 155T763 858.5q-54 54.5-127 86T480 976Zm0-60q142 0 241-99.5T820 576q0-142-99-241t-241-99q-141 0-240.5 99T140 576q0 141 99.5 240.5T480 916Zm0-340Z"/></svg>
                    <svg v-if="article.publishStatus == 'Progress'" class="article-status-checking" xmlns="http://www.w3.org/2000/svg" height="48" viewBox="0 96 960 960" width="48"><path d="M266.118 626Q287 626 301.5 611.382q14.5-14.617 14.5-35.5Q316 555 301.382 540.5q-14.617-14.5-35.5-14.5Q245 526 230.5 540.618q-14.5 14.617-14.5 35.5Q216 597 230.618 611.5q14.617 14.5 35.5 14.5Zm214 0Q501 626 515.5 611.382q14.5-14.617 14.5-35.5Q530 555 515.382 540.5q-14.617-14.5-35.5-14.5Q459 526 444.5 540.618q-14.5 14.617-14.5 35.5Q430 597 444.618 611.5q14.617 14.5 35.5 14.5Zm213 0Q714 626 728.5 611.382q14.5-14.617 14.5-35.5Q743 555 728.382 540.5q-14.617-14.5-35.5-14.5Q672 526 657.5 540.618q-14.5 14.617-14.5 35.5Q643 597 657.618 611.5q14.617 14.5 35.5 14.5ZM480.266 976q-82.734 0-155.5-31.5t-127.266-86q-54.5-54.5-86-127.341Q80 658.319 80 575.5q0-82.819 31.5-155.659Q143 347 197.5 293t127.341-85.5Q397.681 176 480.5 176q82.819 0 155.659 31.5Q709 239 763 293t85.5 127Q880 493 880 575.734q0 82.734-31.5 155.5T763 858.316q-54 54.316-127 86Q563 976 480.266 976Zm.234-60Q622 916 721 816.5t99-241Q820 434 721.188 335 622.375 236 480 236q-141 0-240.5 98.812Q140 433.625 140 576q0 141 99.5 240.5t241 99.5Zm-.5-340Z"/></svg>   
                </div>
            </div>
        </div>
...
const MY_ARTICLES_QUERY = gql`query GetArticleByUser($id: Int!) {
    getArticleByUser(id: $id){
        id
        type
        name
        author {
...
async function getArticles() {
    await apolloClient
        .query({
            query: MY_ARTICLES_QUERY,
            variables: {
                id: user.profileID
            }
        })
        .then(result => { 
            result.data.getArticleByUser.forEach(article => {
                ...
                if (article.author.authorId.id != article.reviewer.id) {
                    ...
                    console.log(reviewerFullName)
                }
                let tagList = []
                article.articletagSet.forEach(tag => {
                    tagList.push(tag.tagId.name)
                })
                let date = new Date(Date.parse(article.creationDate)).toLocaleDateString('ru-RU', {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit',
                })
                let fdate = new Date(Date.parse(article.creationDate))
                articles.push({
                    id: article.id,
                    title: article.name,
                    type: article.type,
